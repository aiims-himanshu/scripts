  ...
  for SUBJ_DIR in $BIDS_ROOT/$dataset/sub-*/anat; do
    ...
    if [[ -f $FLAIR ]]; then
      ...
      $ANTS_REG -d 3 \
        ...
        -s 2x1x0x0vox \
        -f 4x2x1x1 \
        -o "$TEMP_SUBJ/${subj}_FLAIR_to_T1_"

      echo "🎯 Resampling FLAIR to T1w..."
      $ANTS_APPLY -d 3 \
        -i $FLAIR \
        -r $T1 \
        -t "$TEMP_SUBJ/${subj}_FLAIR_to_T1_1Warp.nii.gz" \
        -t "$TEMP_SUBJ/${subj}_FLAIR_to_T1_0GenericAffine.mat" \
        -n BSpline \
    ...
    if [[ -f $T2 ]]; then
      ...
      $ANTS_REG -d 3 \
        ...
        -s 2x1x0x0vox \
        -f 4x2x1x1 \
        -o "$TEMP_SUBJ/${subj}_T2w_to_T1_"

      echo "🎯 Resampling T2w to T1w..."
      $ANTS_APPLY -d 3 \
        -i $T2 \
        -r $T1 \
        -t "$TEMP_SUBJ/${subj}_T2w_to_T1_1Warp.nii.gz" \
        -t "$TEMP_SUBJ/${subj}_T2w_to_T1_0GenericAffine.mat" \
        -n BSpline \
